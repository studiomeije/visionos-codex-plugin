...
- Use [`runtime-logging.md`](runtime-logging.md) for `Logger` categories, privacy annotations,
...
Use the selected Xcode bridge or shell action output as the first source of
truth for:
...
Then use unified logs for `Logger` / `OSLog` events. Launch success does not
prove that a specific unified-log event fired.
...
- The app still builds after instrumentation changes.
