...
description: Implement and debug RealityKit runtime animation, character controllers, skeletal poses, IK, body tracking, blendshapes, AnimationGraphResource, AnimationGraphComponent, retargeting, root motion, navigation, behavior trees, collision, physics bodies, joints, forces, particles, compute simulations, and cloth on visionOS 27. Use when a RealityKit task is primarily about runtime motion, character behavior, physical simulation, hit-test collision shape details, particle simulation, or animation/physics troubleshooting. For authored Reality Composer Pro 3 Animation Graph or Animator Graph project edits, use animationgraph-editor.
---
...
5. Switch to `animationgraph-editor` when the task is authored RCP3
   Animation Graph / Animator Graph project editing inside `.realitycomposerpro`.
6. Switch to `realitykit-visionos-developer` for general input, attachments,
